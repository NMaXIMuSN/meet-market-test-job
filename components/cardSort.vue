<template>
  <v-card class="card">
    <v-list class="card__top">
      <div class="card__top-title">Выберите тип сортировки</div>
    </v-list>

    <v-list class="card__list">
      <v-radio-group
        v-model="radio"
        @change="$store.commit('filters/setCurretnSort', $event)"
      >
        <template v-for="sex in $store.getters['filters/getSort']">
          <div :key="sex">
            <v-radio class="card__radio" :label="sex" :value="sex"></v-radio>
            <v-divider></v-divider>
          </div>
        </template>
      </v-radio-group>
